---
title: 카페24 데이터 수집
date: 2020-10-30 11:10:86
category: etc
draft: false
---

카페 24것도 혹시 몰라서 데이터 백업 해보았다.

```javascript
<!--@layout(/layout/basic/layout.html)-->
<style>
    #foot{padding-bottom:50px}
    .icon_set{bottom:55px}
</style>
<div module="product_detail">
    ...
    <p module="product_additional" class="simple_txt {$simple_desc|display}">{$simple_desc}</p>
    <!-- 가격 정보 -->
    <!--@js(/awesome/js/p_price.js)-->
    <div module="product_image" class="detail_top_price">
        <p class="p_custom {$price_custom_display}"><span class="price">{$product_custom}</span>원</p>
        <p class="p_price {$product_price_display}"><span class="price">{$product_price}</span>원 <span class="p_discount"></span></p>
    </div>
    ...
    <div class="prdDesc">
                    ...
                    <tr module="product_quantity" class="quantity">
                        ...
                        <td>
                            <div class="ec-base-qty">
                                <img class="{$quantity_down_class}" alt="down" src="//img.echosting.cafe24.com/skin/mobile/common/ico_quantity_down.jpg" width="33" height="29">
                                {$form.quantity}
                                <img class="{$quantity_up_class}" alt="up" src="//img.echosting.cafe24.com/skin/mobile/common/ico_quantity_up.jpg" width="33" height="29">
                            </div>
                        </td>
                    </tr>
                </tbody>
        ...
        <div class="ec-product-coupon"></div>
        ...
        <div module="product_action" id="{$fixed_action_button}">
            <div class="ec-base-button {$action_display|display}">
                <a href="#none" class="btn_buy {$buy_display|display}" onclick="{$action_buy}"><span id="{$btn_buy_mobile_id}">바로 구매하기</span><span class="{$btn_reserve_class|display}" id="{$btn_reserve_mobile_id}">예약주문</span></a>
                <button type="button" class="btn_detail {$basket_display|display}" onclick="{$action_basket}" id="actionCart">장바구니</button>
                <button type="button" class="btn_detail {$wishlist_display|display}" onclick="{$action_wishlist}" id="actionWish">관심상품</button>
            </div>
            <div class="ec-base-button gColumn soldout {$soldout_display|display}">
                <button type="button" class="btn_detail soldOut">품절</button>
                <button type="button" class="btn_detail {$wishlist_display|display}" onclick="{$action_wishlist}" id="actionWishSoldout">관심상품</button>
            </div>
            <!-- 네이버 체크아웃 구매 버튼  -->
            <div id="NaverChk_Button" style="display:none;"></div>
        </div>

        <div module="product_action" id="orderFixArea">
            <div class="ec-base-button {$action_display|display}">
                <a href="#none" class="btn_buy {$buy_display|display}"><span id="{$btn_buy_mobile_clone_id}">BUY NOW</span><span class="{$btn_reserve_class|display}" id="{$btn_reserve_mobile_clone_id}">예약주문</span></a>
                <button type="button" class="btnNormal {$basket_display|display}" id="actionCartClone">ADD CART</button>
                <button type="button" class="btnNormal {$wishlist_display|display}" id="actionWishClone">WISH</button>
            ...
            <div class="ec-base-button gColumn {$action_soldout_display|display}">
                ...
                <button type="button" class="btnNormal {$wishlist_display|display}" id="actionWishSoldoutClone">WISH</button>
            </div>
        </div>
...
<div module="product_additional" class="detailWrap">
    <!--@css(/css/module/product/additional.css)-->
    <!--@css(/css/module/product/additional_animation.css)-->
	<!--@js(/awesome/js/detail_tab.js)-->

    <ul class="detail_tab">
        <li data-tab="tab-1" class="on"><a href="javascript:;">상품상세정보</a></li>
        <li data-tab="tab-2"><a href="javascript:;">상품구매안내</a></li>
        <li data-tab="tab-3"><a href="javascript:;">사용후기 <span class="count">{$review_count}</span></a></li>
        <li data-tab="tab-4"><a href="javascript:;">상품Q&amp;A <span class="count">{$qna_count}</span></a></li>
    </ul>

    <!-- 상세정보 -->
    <div id="tab-1" class="detail_con on">
    ...
    <!-- 구매안내 -->
    <div id="tab-2" class="detail_con">
        <div class="contArea">
            <p class="contBox">
                <span class="con_tit">PAYMENT INFO</span>
                {$payment_info}
            </p>
            <p class="contBox">
                <span class="con_tit">DELIVERY INFO</span>
                배송 방법 : {$shipping_method}<br/>
                배송 지역 : {$shipping_area}<br/>
                배송 비용 : {$shipping_fee}<br/>
                배송 기간 : {$shipping_period}<br/>
                배송 안내 : {$shipping_info}
            </p>
            <p class="contBox">
                <span class="con_tit">EXCHANGE &amp; RETURN</span>
                {$exchange_info}
            </p>
            <p class="contBox">
                <span class="con_tit">SERVICE INFO</span>
                {$service_info}
            </p>
        </div>
    </div>
    ...
    <!-- 상품후기 -->
    <div id="tab-3" class="detail_con">
        <h3 class="tit_common">REVIEW<span>구매하신 상품과 관련된 솔직한 후기를 남겨주세요</span></h3>
        <div class="board" id="use_review">
    ...
    <!-- 상품문의 -->
    <div id="tab-4" class="detail_con">
        <h3 class="tit_common">Q&amp;A<span>상품 관련 궁금한 점이 있으신가요? 문의글을 남겨주세요</span></h3>
        <div class="board" id="use_qna">
...
<script src="/web/meogbang/js/jquery-3.3.1.min.js"></script>
<script type="text/javascript">
window.jQuery3_3_1 = jQuery.noConflict(true);
</script>
<script>
...
jQuery3_3_1(document).ready(function() {
	...
	console.log("refererType: "+refererType);
	...
	jQuery3_3_1.ajax({
		...
		success : function(data) {
			...
			ip = data.ip;
			console.log("ip: "+ip);
			param.refererType = refererType;
			param.ip = ip;
			param.deviceType = "MOBILE";
    		param.productName=newData[1].productName;
    		param.productPrice=newData[2].productPrice;
    		param.member=newData[3].member;
    		param.memberID=newData[4].memberID;
					console.log('sss',param,newData)
              //  console.log('dd',newData.length)
			jQuery3_3_1.ajax({
				type : "GET",
				dataType : "json",
				crossDomain : true,
				data : param,
				url : "http://13.209.108.25:8080/store/referer/insert",
				success : function(data) {
					console.log(data);
...
jQuery3_3_1(document).on("click", ".btn_buy", function(){
	console.log("click mobile npay4");
	var price = jQuery3_3_1(".totalPrice .total strong").text();
	var product = jQuery3_3_1(".xans-product-detail h1.name").text();

	var updatePParam = {};
	updatePParam.refererType = refererType;
	updatePParam.ip = ip;
	updatePParam.buyType = "일반구매";
	updatePParam.productName = product;
	updatePParam.productPrice = price;
	console.log("updatePParam");
	console.log(updatePParam);
	...
	jQuery3_3_1.ajax({
		...
		crossDomain : true,
		data : updatePParam,
		url : "http://13.209.108.25:8080/store/buy/update",
		success : function(data) {
			console.log(data);
		}
	});
...
jQuery3_3_1(document).on("click", ".npay_btn_list .npay_btn_item", function(){
	console.log("click mobile npay5");
	var d = new Date();
	var price = jQuery3_3_1(".total strong").text();
	console.log("price =" +price);
	/* var product = $("#totalProducts > table > .option_products > .option_product > td > p").text(); */
	var product = jQuery3_3_1(".xans-product-detail .name").text();
	console.log("product =" +product);

	var updateParam = {};
	updateParam.refererType = refererType;
	updateParam.ip = ip;
	updateParam.buyType = "네이버페이";
	updateParam.productName = product;
	updateParam.productPrice = price;
...
<script>
jQuery3_3_1(document).on("click", "#actionCart", function(){
...
jQuery3_3_1(document).on("click", "#actionCartClone", function(){
	console.log("장바구니버튼 클릭");
    return;
	var updateParam = {};
	updateParam.refererType = refererType;
...
<script>
jQuery3_3_1(document).on("click", "#actionWish", function(){
	console.log("관심상품 버튼 클릭");
    return;
	var updateParam = {};
	updateParam.refererType = refererType;
	updateParam.ip = ip;
...
jQuery3_3_1(document).on("click", "#actionWishClone", function(){
	console.log("관심상품 버튼 클릭");
    return;
	var updateParam = {};
	updateParam.refererType = refererType;
