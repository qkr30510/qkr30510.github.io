...
함수 선언식인 경우에만 적용된다. 호이스팅에 관해 설명하기 전에 먼저 함수 선언식 및 함수 표현식에 대해 이해하고 가자.

- 함수 선언식
  function 명령어와 함께 함수명을 지정하고 function body 내용을 구성한다.
...
**호이스팅**은 들어올리다 라는 의미를 가지고 있다.
javascript에서도 호이스팅은 **코드에 선언된 변수 및 함수를 코드 상단으로 끌어올리는 것**을 말하며 이는 _변수 번위가 전역 범위인지 함수 범위인지에 따라 다르게 수행될 수 있다._
함수 내에서 선언한 **함수 범위의 변수는, 해당 함수의 최상위로 함수 밖에서 선언한 전역 범위의 전역 변수는 스크립트 단위의 최상위로 끌어올려진다.** 주의해야할 점은 _변수의 선언과 할당 내용 모두를 상단으로 끌어올리는게 아니라, 선언부분만 분리하여 최상위로 끌어올린다._ 아래 예제를 확인해보자.
...
위의 코드를 확인해보면 `var name = "ojava";`를 `var name;` 과 `name = "ojava"`로 분리하여 변수 선언부를 함수 상단으로 끌어올려서 선언해버리므로 오류가 발생하지 않는 대신 값이 정의되지 않았다는 의미로 `undefined`를 반환하는 것을 알 수 있다. 즉 아래 코드와 같이 해석됨을 알 수 있었다.
...
만약 변수선언에 `var`를 생략하는 경우 예상치 못한 결과가 나올 수 있다. 아래 코드를 확인해 보자.
...
오류가 발생하지도 않고, 변수를 할당하기 전임에도 이미 할당된 값으로 반환해버린다.
**함수 내에서 변수 선언 명령어를 제외하고 선언 시 전역 변수의 형태로 사용됨에 주의하자.**

출처: https://ojava.tistory.com/144
...
// 결과: 10만 10번 출력됨
// 이유: setTimeout()에 인자로 넘긴 익명함수는 모두 0.1초 뒤에 호출될 것이다. 그 0.1초 동안에 이미 반복문이 모두 순회되면서 i값은 이미 10이 된 상태이다. 그 때 익명 함수가 호출되면서 이미 10이 되어버린 i를 참조한다.
...
-> 순차적으로 이벤트를 실행하는것을 event loop라고 한다.
...
- React는 애초부터 역할군을 분리하여 개발하기 때문에, 이런 점에서 설계자, 디자이너와 프론트의 역할이 아주 확실히 구분되어 있는 점
- Vue는 반대로 구분 없이 컴포넌트 정의 안에서 뷰와 모델, 그리고 디자인이 하나의 확장자 Vue로 뭉쳐져 있다.
...
우회방법: 서버에서 CORS 요청 핸들링하기. 즉, 모든 요청을 허용하는 방식이다.
